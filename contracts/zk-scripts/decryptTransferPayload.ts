...
if (
  ...
  console.error(
    "Usage: tsx zk-scripts/decryptTransferPayload.ts <recipientPrivLo> <recipientPrivHi> <auditPrivLo> <auditPrivHi> <ephemeralPubX> <ephemeralPubY> <cipherNonce> <encryptedRecipientPayload> <encryptedAuditPayload>"
  );
  process.exit(1);
}
...
const kRecipient = await sharedKeyFromPub(ephemeralPubX, ephemeralPubY, recipientPrivLo, recipientPrivHi);
const kAudit = await sharedKeyFromPub(ephemeralPubX, ephemeralPubY, auditPrivLo, auditPrivHi);

const recipientMask = await poseidon([kRecipient, cipherNonce]);
const auditMask = await poseidon([kAudit, cipherNonce]);

const recoveredRecipient =
  (toField(encryptedRecipientPayload) - toField(recipientMask) + MOD) % MOD;
const recoveredAudit =
  (toField(encryptedAuditPayload) - toField(auditMask) + MOD) % MOD;

const encoded = AbiCoder.defaultAbiCoder().encode(
  ["uint256", "uint256"],
  [recoveredRecipient.toString(), recoveredAudit.toString()]
);
